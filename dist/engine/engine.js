...
function firstActiveFrom(state, start) {
    let seat = start;
    for (let i = 0; i < 6; i += 1) {
...
function resetStreet(state, config) {
    for (const player of state.players) {
        player.streetCommitted = 0;
    }
    state.canRaise = state.players.map((player) => player.status === "active" && player.stack > 0);
    state.currentBet = 0;
...
function dealToSeat(state, seat, count) {
    const player = state.players[seat];
...
function computePots(players) {
    ...
    const uniqueLevels = Array.from(new Set(commitments.filter((v) => v > 0))).sort((a, b) => a - b);
    const pots = [];
    let previous = 0;
...
function distributeOddChips(total, winners, dealerSeat) {
...
function collectUncontested(state) {
...
function ensureHandSetup(state, config, events) {
...
function isBettingRoundComplete(state) {
    const activePlayers = state.players.filter((p) => p.status === "active" && p.stack >= 0);
    if (activePlayers.length === 0) {
        return true;
    }
    return activePlayers.every((p) => p.streetCommitted === state.currentBet);
}
function advancePhase(state, config, events) {
...
function resolveHandEnd(state, events) {
...
function advanceAfterAction(state, config, events) {
...
function legalActionsForPlayer(state, config) {
...
function applyPlayerAction(state, config, action) {
    const player = getPlayerToAct(state);
    ...
    if (action.type === "bet") {
        ...
        state.canRaise = state.players.map((p) => p.status === "active" && p.stack > 0);
        return;
    }
